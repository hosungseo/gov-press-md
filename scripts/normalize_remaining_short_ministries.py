...
for name, count in ministry_counts.items():
    if name and len(name) >= 3 and count >= 20:
...
changes = []
for rec in records:
    current = rec['meta'].get('ministry', '')
    if not current or len(current) > 2:
        continue

    title = rec['meta'].get('title', '')
    news_id = rec['meta'].get('news_item_id', '')
    inferred = infer_from_text(title, rec['body_lines'])
    new_ministry = inferred or '미분류'
    reason = 'inferred' if inferred else 'fallback-unclassified'
...
manifest_path = QUALITY_DIR / 'remaining-short-ministry-normalization-manifest.csv'
...
for (old_m, new_m, reason), count in summary_counts.most_common(60):
    summary_lines.append(f'- {old_m} → {new_m} ({reason}): {count}건')
summary_path = QUALITY_DIR / 'remaining-short-ministry-normalization-summary.md'
